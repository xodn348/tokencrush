...
class TestIntegration:
    ...
    def test_chat_workflow(self, mock_env_keys):
        ...
        from tokencrush.cli import app
        from tokencrush.sdk import SDKStats

        with patch("tokencrush.cli.TokenCrush") as mock_tc:
            mock_instance = MagicMock()
            mock_instance.chat.return_value = "This is the LLM response."
            mock_instance.stats.return_value = SDKStats(
                cache_hit_rate=0.75, total_queries=10, cached=7, cost_saved=0.0
            )
            mock_tc.return_value = mock_instance
    ...
    def test_compress_with_rate(self):
        ...
        with patch("tokencrush.cli.TokenCompressor") as mock_tc:
            ...
            result = runner.invoke(
                app, ["compress", "A very long text " * 50, "--rate", "0.2"]
            )

            assert result.exit_code == 0
            mock_instance.compress.assert_called_once_with(
                "A very long text " * 50, rate=0.2
            )
